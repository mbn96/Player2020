...
import static mbn.packfragmentmanager.fragmanager.CustomFragmentSwipeBackAnimator.ANIM_DEFAULT;
...
public abstract class BaseFragment {
    ...
    public void destroy() {
        ...
        fragmentSwipeBackManager.removeFragmentView(getView());
        context = null;
    ...
    public void setToolbarScrollFraction(float fraction) {
        if (hasAppBar())
    ...
    protected void calculateAndSetTopPadding() {
        if (hasAppBar())
            view.setPadding(0, getToolbarHolder().calculateHeight(), 0, 0);
    }
    ...
    private void createView(@NonNull LayoutInflater inflater, @Nullable ViewGroup container) {
        ...
        BaseFrameLayoutForFrags frameLayoutForFrags = new BaseFrameLayoutForFrags(getContext()) {
            @Override
            void layoutWithSize() {
                isLayoutDone = true;
                checkForAnim();
