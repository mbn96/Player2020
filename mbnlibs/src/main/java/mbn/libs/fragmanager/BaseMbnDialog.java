...
import static mbn.libs.fragmanager.CustomFragmentSwipeBackAnimator.ANIM_DIALOG_RTL;
...
//        dialogBase.setBackgroundColor(Color.argb((int) ((255 / 2) * (1 - fraction)), 0, 0, 0));
        ...
        if (backBlur != null && !(this instanceof BasicMbnPopupMenu)) {
            backBlur.setAlpha((float) Math.sqrt(1 - fraction));
        }
//        getFragmentSwipeBackManager().setContentBlurProgress(1 - fraction);
        if (darkener != null) {
            darkener.setAlpha((1f / 4) * (1 - fraction));
        }
...
//    }
    ...
    public void invalidateBackBlur() {
        if (getView() != null && getFragmentManager() != null && isLayoutDone && getFragmentSwipeBackManager().getWidth() > 0)
            Effects.drawViewTree(getFragmentSwipeBackManager(), dialogBase, 0.4f, true, 40, null, (result, info) -> {
                if (!isDestroyed()) backBlur.setImageBitmap((Bitmap) result);
            });
    }
