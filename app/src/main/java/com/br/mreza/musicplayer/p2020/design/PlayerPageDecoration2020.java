...
public class PlayerPageDecoration2020 extends pageDecorationMaterialTheme {

    public PlayerPageDecoration2020(Context context, View topMovingView, RecyclerView recyclerView, HeightChangerView background) {
        ...
        topMovingView.setOnTouchListener(new View.OnTouchListener() {
            ...
            public boolean onTouch(View v, MotionEvent event) {
                return !shouldMove(recyclerView);
            }
        });
...
//            Bitmap out = MbnUtils.stackBlur(request.getBitmap(), 20, false);
            ...
            request.getCanvas().drawColor(accent);
            return out;
        }
    ...
    private Disposal<CaptureBitmap> bitmapSimplePool = new Disposal<CaptureBitmap>(15) {
        ...
        public CaptureBitmap createNew() {
            return new CaptureBitmap((int) (topMovingView.getWidth() / scale), (int) (topMovingView.getHeight() / scale), scale);
        }
...
//        super.setAccent(back);
        this.accent = back;
        imageView.setImageTintList(ColorStateList.valueOf(mbn.libs.imagelibs.imageworks.MbnUtils.alphaChanger(back, 160)));
//        alphaAccent = MbnUtils.colorAlphaChanger(MbnUtils.colorWhitener2(accent, 30), 130);
...
//        }
    ...
    protected void doTheScaling(Canvas canvas, RecyclerView parent) {
        ...
        /*
          This is disabled for new design.
         */
        /*
        if (inOverDraw) {
...
//                int screenHeight = shouldMove(parent) ? parent.getChildAt(0).getTop() - topMovingView.getHeight() : 0;
    ...
    private boolean shouldRemakeBitmap(CaptureBitmap captureBitmap) {
        return !(captureBitmap.getHeight() == (int) (topMovingView.getHeight() / scale) && captureBitmap.getWidth() == (int) (topMovingView.getWidth() / scale));
    }
