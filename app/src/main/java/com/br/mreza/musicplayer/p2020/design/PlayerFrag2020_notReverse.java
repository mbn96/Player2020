...
import com.br.mreza.musicplayer.p2020.design.tests.NewSeekbarPreviewFrag;
...
public class PlayerFrag2020_notReverse extends BaseFragment implements View.OnClickListener,
        View.OnLongClickListener,
        PlayerCommunicates.PlayerCallbacks,
        MusicInfoHolder.CallBack, A_B_Manager.A_B_Interface {
...
//    private VisualizerNewDesign visualizerNewDesign;
    ...
    private PlayerPageViewPager2020 playerPageViewPager;
    private PlayerPagePagerAdapter2020 playerPagePagerAdapter;
    private LinearLayout mainTop;
    private View statusBarShade;
    private ImageButton findNowPlaying;
...
//    }
    ...
    public void onProfileItemChanged(ThemeCoordinator.DefaultThemeTypes type, ThemeCoordinator.ColorItem changed) {
        ...
        switch (type) {
            case BackgroundColor:
                if (pageDecoration instanceof PlayerPageDecoration2020) {
                    ((PlayerPageDecoration2020) pageDecoration).setAccent(accent, back = changed.getValue());
                }
                album_big.setImageTintList(ColorStateList.valueOf(MbnUtils.colorAlphaChanger(back, 220)));
                statusBarShade.setBackgroundTintList(ColorStateList.valueOf(MbnUtils.colorAlphaChanger(back, 210)));
                play_Button.setPlayerIconColor(changed.getValue());
                break;
            case AccentColor:
                ...
                break;
            case TitleTextColor:
                adapter.setTitleColor(changed.getValue());
                break;
            case ContextTextColor:
                setContextColors(changed.getValue());
                break;
        }
    }

    private void setContextColors(int color) {
        artist.setTextColor(color);
        time.setTextColor(color);
        ColorStateList stateList = ColorStateList.valueOf(color);
        findNowPlaying.setImageTintList(stateList);
        goToTop.setImageTintList(stateList);
        option_button.setImageTintList(stateList);
        next_Button.setImageTintList(stateList);
        previous_Button.setImageTintList(stateList);
        adapter.setContextColor(color);
    ...
    public View onCreateViewForChild(@NonNull LayoutInflater inflater, @Nullable ViewGroup container, @Nullable Bundle savedInstanceState) {
        ...
        return inflater.inflate(R.layout.player_page_2020, container, false);
    }
...
//    }
    ...
    public void onViewCreated(@NonNull final View view, @Nullable Bundle savedInstanceState) {
        ...
        playerPagePagerAdapter = new PlayerPagePagerAdapter2020(DataBaseUtils.defaultArt, playerPageViewPager, view.findViewById(R.id.frame_layout_top));
        playerPageViewPager.setAdapter(playerPagePagerAdapter);

        mainTop = view.findViewById(R.id.player_frag_linear_top);
        statusBarShade = view.findViewById(R.id.status_shade);
...
//        if (themeType != 0) album_big.setMaterial(true);
        ...
        getUserView().postDelayed(() -> setCanStartAnim(true), 50);
    }

    @Override
    protected void statusBarSizeChanged() {
        super.statusBarSizeChanged();
        mainTop.setPadding(0, (int) getFragmentSwipeBackManager().getStatusBarSize(), 0, 0);
        statusBarShade.getLayoutParams().height = (int) getFragmentSwipeBackManager().getStatusBarSize();
...
//        ShadowBackground shadowBackground = new ShadowBackground(getContext(), bitrate, 45, (int) (10 * getDisplayDensity()), Color.RED);
//        bitrate.setBackground(shadowBackground);
    ...
    private void arrangeButtons(View view) {
        ...
        findNowPlaying = view.findViewById(R.id.player_frag_find_button);
        goToTop = view.findViewById(R.id.player_frag_go_top_button);

//        ShadowBackground_2 nextButtonBack = new ShadowBackground_2(getContext(), next_Button, 45, (int) (30 * getDisplayDensity()), Color.LTGRAY);
//        next_Button.setBackground(nextButtonBack);
//
//        ShadowBackground previousButtonBack = new ShadowBackground(getContext(), previous_Button, 45, (int) (12 * getDisplayDensity()), Color.LTGRAY);
//        previous_Button.setBackground(previousButtonBack);
...
//                        getFragmentSwipeBackManager().addFragment(new VisualizerFrag_1());
                        getFragmentSwipeBackManager().addFragment(Test_streamFrag.newInstance(currentSong.getPath()));
//                        getFragmentSwipeBackManager().addFragment(NewSeekbarPreviewFrag.newInstance(currentSong.getPath(), currentID));
//                        currentQueueList.smoothScrollToPosition(0);
                        /*
                        File output = new File(Environment.getExternalStoragePublicDirectory(Environment.DIRECTORY_MOVIES), "output" + System.currentTimeMillis() + ".mp3");
                        File input = new File(currentSong.getPath());
                        Log.i(TAG, "onClick: " + input.getAbsolutePath());
                        int rc = FFmpeg.execute("-i " + JavaUtils.decode_UTF_8(JavaUtils.encodeTo_UTF_8(input.getAbsolutePath())) + " -ab 96 " + output.getAbsolutePath());
    ...
    private void makeTheList(View view) {
        ...
        switch (themeType) {
            case 0:
                ...
                break;
            case 1:
                pageDecoration = new PlayerPageDecoration2020(getContext(), view.findViewById(R.id.frame_layout_top), currentQueueList, playerPageViewPager);
...
//        seekBar.setMax(durationThis);
    ...
    public void onResume() {
        super.onResume();
//        getActivity().setUI_style(true, false);
