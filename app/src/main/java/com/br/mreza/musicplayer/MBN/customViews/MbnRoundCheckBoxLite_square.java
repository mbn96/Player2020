...
public class MbnRoundCheckBoxLite_square extends androidx.appcompat.widget.AppCompatCheckBox {
...
//    private Paint paintForDraw;
    ...
    public MbnRoundCheckBoxLite_square(Context context) {
        super(context);
        init();
    }

    public MbnRoundCheckBoxLite_square(Context context, AttributeSet attrs) {
        super(context, attrs);
        init();
    }

    public MbnRoundCheckBoxLite_square(Context context, AttributeSet attrs, int defStyleAttr) {
...
//        pathForOutline.addCircle(w / 2, h / 2, ((w / 2) - (displayDensity * factor - 9)) * (value + 0.2f), Path.Direction.CCW);

        float remainder = (w - (10 * displayDensity)) / 2;

        float calculate = (remainder - ((remainder - ((w / displayDensity) / 7)) * value));

        pathForOutline.addRoundRect(calculate, calculate,
                h - calculate, w - calculate,
                10 * displayDensity, 10 * displayDensity, Path.Direction.CCW);


//        pathForFill.addCircle(w / 2, h / 2, ((w / 2) - (displayDensity * factor - 9)) * (value + 0.2f), Path.Direction.CCW);

        pathForFill.addRoundRect(calculate, calculate,
                h - calculate, w - calculate,
                10 * displayDensity, 10 * displayDensity, Path.Direction.CCW);
